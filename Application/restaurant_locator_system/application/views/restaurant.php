...
<?php
    ...
    $dirName = "C:/xampp/htdocs/restaurant_locator_system/res/images/restaurants/{$_GET['location']} City/{$_GET['name']}/";
    $supportedFormat = array('jpg', 'jpeg', 'png', 'gif', 'bmp');

    $scanDir = scandir($dirName);

    //Collect all image files in localhost directory and display//
    /*foreach($scanDir as $img){
        $scanDir = pathinfo($img, PATHINFO_EXTENSION);
        if(in_array($img, $supportedFormat) == TRUE){
            echo "<img src='{$dirName}{$img}'>";
        }
    }*/

    echo "<img src='http://localhost/restaurant_locator_system/res/images/restaurants/{$_GET['location']} City/{$_GET['name']}/image.jpg' width='150px' height='150px'>";
    echo $_GET['name'];
?>
